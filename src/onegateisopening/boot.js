...
(function() {
  ...
  var insertScript = function(url) {
    // XXX fail to load should STOP
    Spitfire.loader.script(url);
  };

  var insertStyleSheet = function(url) {
    var h = document.getElementsByTagName('head')[0];
    var s = document.creatElement('link');
  ...
  var getPackedObjects = function(pack, version, sub, useFull) {
    var v = pack.filter(function(item) {
      if (sub && !(new RegExp(sub)).test(item))
        return;
      if (version == 'trunk')
        return !!item.match(/-trunk\./);
      if (version)
        return (new RegExp(version)).test(item);
      return !item.match(/-trunk\./);
    });
   ...
   */
  jsBoot.core.SHIMS = 'shims-stack';
  jsBoot.core.EMBER_STACK = 'ember-stack';
  jsBoot.core.TOOLING_STACK = 'tooling-stack';

  // Shortcut to request trunk versions for something
  jsBoot.core.TRUNK = 'trunk';
  ...
  jsBoot.core.debug = new (function() {
    ...
    var reloadStyleSheets = function() {
      ...
      reloadTout = window.setTimeout(reloadStyleSheets, 100);
    };

    this.toggle = function(withAutoReload) {
      debug = !debug;
      if (debug) {
        if (withAutoReload)
          reloadStyleSheets(100);
        return;
      }
      window.clearTimeout(reloadTout);
    };
   ...
   */
  jsBoot.core.boot = function(cbk, trunk) {
    this.use(this.SHIMS);
    this.wait();
   ...
   * Shimit main
   */
  // jsBoot.core.useShims = function(withUnsafe) {
  ...
  //     Spitfire.loader.script(spitfireLink + shims[x]);
  //   return this;
  // };
   ...
   * Use main
   */
  jsBoot.core.use = function(thing, version, sub) {
    switch (thing) {
      case this.SHIMS:
        // This is undocumented - allows to use placeholders shims - will fuck up feature detection
        if (version)
  ...
  jsBoot.core.getScriptBaseUrl = function(scriptName) {
    if (!scriptName)
